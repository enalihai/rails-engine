...
RSpec.describe 'Item API' do
  ...
  xdescribe 'POST requests' do
    ...
    it 'deletes an invoice after destroy if no items remain' do
      merchant = create(:merchant)
      item_v1 = merchant.items.create!({
        name: "Slow-Crete",
    ...
    it 'can update an items attributes' do
      ...
      first_name = item_v1.name
      first_description = item_v1.description
      first_unit_price = item_v1.unit_price
      ...
      patch "/api/v1/items/#{item_v1.id}", headers: headers, params: JSON.generate(item: name_patch)

      expect(response.status).to eq(200)
      expect(item_v2.name).to_not eq(first_name)
      expect(item_v2.name).to eq('Quick-Crete')
    end
    ...
    it 'returns a 404 if update doesnt correct patch' do
      ...
      patch "/api/v1/items/#{item.id}", headers: headers, params: JSON.generate(item: patch_params)
